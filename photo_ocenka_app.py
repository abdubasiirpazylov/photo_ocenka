...
# --- УМНОЕ ВПИСЫВАНИЕ (БЕЗ ОБРЕЗКИ И ИСКАЖЕНИЙ) ---
def process_and_fit_image(uploaded_file):
    ...
    max_dim = max(width, height) # Берем самую большую сторону
    
    # Создаем идеальный белый квадрат
    square_img = Image.new('RGB', (max_dim, max_dim), (255, 255, 255))
    
    # Вставляем оригинальное фото ровно по центру белого квадрата
    left = (max_dim - width) // 2
    top = (max_dim - height) // 2
    square_img.paste(image, (left, top))
    
    img_byte_arr = io.BytesIO()
    square_img.save(img_byte_arr, format='JPEG', quality=85)
    img_byte_arr.seek(0)
    return img_byte_arr
...
if uploaded_photos:
    for uf in uploaded_photos:
        if uf.file_id not in st.session_state.processed_ids:
            # Используем новую функцию вписывания
            fixed_bytes = process_and_fit_image(uf)
...
if photo_data_list:
    if st.button("СГЕНЕРИРОВАТЬ ФОТООТЧЕТ", type="primary", use_container_width=True):
        try:
            ...
            # --- НОВАЯ СИСТЕМА ИМЕН ФАЙЛОВ С ДАТОЙ ---
